	...
	override public function create():Void 
	{
		...
		for (i in 0...Data.amtLevels) {
			if (Data.canPlayLevel[i + 1]) {
				_btnLevel = new FlxButton(offset * (i % lvl_per_row + 4), offset * (Std.int(i / lvl_per_row) * 1.25 + 2),  i + 1 + "", clickLevel.bind(i+1));
				_btnLevel.scale.set(0.5, 2);
				_btnLevel.label.setFormat("", 15);
				add(_btnLevel);
				if (Data.completedLevel[i + 1]) {
					_txtBestTime = new FlxText(_btnLevel.x + 20, _btnLevel.y + 30, offset, Data.bestTimes[i + 1] + "s", 10);
					add(_txtBestTime);
				}
			} else {// FlxText does not align with FlxButton, so the offset is hardcoded
				_txtCannotPlay = new FlxText(offset * (i % lvl_per_row + 4.5) + 5, offset * (Std.int(i / lvl_per_row) * 1.25 + 2) + 5, offset, i + 1 + "", 15);
				add(_txtCannotPlay);
			}
		}
